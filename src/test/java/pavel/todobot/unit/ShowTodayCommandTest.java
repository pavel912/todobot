...
public class ShowTodayCommandTest {
    ...
    @BeforeEach
    void beforeEach() {
        createCommand = applicationContext.getBean(LogCommand.class);
        showCommand = applicationContext.getBean(ShowTodayCommand.class);
    ...
    void testShowEmpty() {
        String response = showCommand.execute(user, messageHandler);
        assertThat(response).isEqualTo("Сегодня еще не было залогировано ни одной задачи");
    }
