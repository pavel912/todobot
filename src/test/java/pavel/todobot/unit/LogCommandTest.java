...
public class LogCommandTest {
    ...
    @BeforeEach
    void beforeEach() {
        command = applicationContext.getBean(LogCommand.class);
    ...
    void testCreate() {
        ...
        assertThat(response).isEqualTo("Новый список успешно создан");
    }

    @Test
    void testUpdate() {
        String createResponse = command.execute(user, message, messageHandler);
        assertThat(createResponse).isEqualTo("Новый список успешно создан");

        String updateResponse = command.execute(user, message, messageHandler);
        assertThat(updateResponse).isEqualTo("Сегодняшний список успешно обновлен");
    }
